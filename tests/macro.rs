...
fn count_matches(rx: &str, g: &Grammar) -> usize {
    Regex::new(rx)
        .unwrap()
        .find_iter(&g.rx().unwrap().to_string())
        .count()
}
...
fn no_unicode() {
    let matcher = grammar! {
        TOP       => r(r"\A") <digits>+ r(r"\z")
        digits    => <ascii> | <non_ascii>
        ascii     => (?-u) r(r"\d+")
        non_ascii => (?u)  r(r"[\d&&[^0-9]]+")
    }
    .matcher()
    .unwrap();
    let p = matcher.parse("123٤५৬789૦").unwrap();
    let ascii = p.all_names("ascii");
